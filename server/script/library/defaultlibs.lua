...
defaultLibs.customType.WithMeta.generics[1].replace = {defaultLibs.customType.WithMeta.value[1]}
defaultLibs.customType.WithMeta.generics[2].replace = {defaultLibs.customType.WithMeta.value[2]}
...
function defaultLibs.init()
    if defaultLibs.initialized then return end

    local libDoc = loadLibLocale("en-US")
    if lang.id ~= "en-US" then loadLibLocale(lang.id, libDoc) end
    ...
    local data = {
        [defaultLibs.global] = "env.luau",
        [defaultLibs.testez] = "3rd/testez.luau"
    }

    for tbl, file in pairs(data) do
        ...
        if state then
            ...
            for _, g in ipairs(env.value) do
                ...
                if g.key[1] == "setmetatable" then g.value.argCount = 2 end
    ...
    defaultLibs.global["type"].value.enums = typeEnums
    defaultLibs.object = {}

    for tp in pairs(defaultLibs.primitiveTypes) do
        defaultLibs.object[tp] = { child = {} }
    end

    for _, field in ipairs(defaultLibs.global.string.value) do
        ...
        defaultLibs.object.string.child[#defaultLibs.object.string.child + 1] = field
    end
            ...
            if state.ast.types then
                for _, alias in ipairs(state.ast.types) do
                    ...
                    if alias.export then
                        defaultLibs.customType[alias.name[1]] = alias
                    end
                end
            end
        end
