...
local function isValidPath(path)
    ...
    if normalizePath(path):match("^%/?[%w_]") then return true end
end

local function inspectModel(model)
    if not model.Children then return end
    ...
    if fs.is_directory(path) then
        ...
        if fs.exists(path / "default.project.json") then
            ...
            if success and project.tree then
                ...
                if project.name then parent.name = project.name end
            end
        ...
        for childPath in fs.pairs(path) do
            ...
            elseif childName == "init.meta.json" then
                ...
                if success and meta.className then parent.value[1] = meta.className end
...
function rojo:matchLibrary(uri)
    if not config.config.intelliSense.autoDetectLibraries then return end
    if uri:match("%.spec%.lua[u]?$") then return nil end
...
function rojo:projectChanged(change)
    ...
    if change.uri:match(projectFileName.."%.project%.json$") then return true end

    local path = furi.decode(change.uri)
    if path then
        local filename = fs.path(path):filename():string()

        if change.type ~= define.FileChangeType.Changed then return false end
        if filename:match("%.meta%.json$") then return false end
        if filename:match("%.model%.json$") then return false end
        if filename:match("%.project%.json$") then return false end
...
function rojo:parseDatamodel()
    if self.DataModel then return inspectModel(self.DataModel) end
end

function rojo:getSourceMap(sourceMap, node, path)
    if not node.value.child then return sourceMap end
...
function rojo:getRojoPath()
    if config.config.workspace.rojoExecutablePath ~= "" then return config.config.workspace.rojoExecutablePath end
    if self.RojoPath then return self.RojoPath end
...
function rojo:parseProject(projectPath, forceDisable)
    if config.config.workspace.useRojoSourcemap and not forceDisable then
        local success, sourceMap = pcall(function()
            ...
            if sourceMap == "" then error(process.stderr:read("a")) end

            return json.decode(sourceMap)
        end)

        if success and sourceMap then return { value = self:buildInstanceTree(sourceMap).value } end
...
function rojo:loadRojoProject()
    ...
    if #rojoProjects == 0 then return end
