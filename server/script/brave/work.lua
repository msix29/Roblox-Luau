...
brave.on("loadProto", function()
    ...
    while true do
        local proto, error = jsonrpc.decode(io.read, log.error)
        if not proto then return brave.push("protoerror", error) end
...
brave.on("compile", function(text)
    local state, error = parser:compile(text, "lua")
    if not state then return log.error(error) end
