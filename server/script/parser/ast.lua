...
local function checkMissEnd(start)
    if not State.MissEndErr then return end
...
local function getValue(values, i)
    ...
    if not value then
        ...
        if last.type == "call" or last.type == "varargs" then return getSelect(last, i - #values + 1) end

        return nil, nil
    end

    if value.type == "call" or value.type == "varargs" then value = getSelect(value, 1) end
...
local function tableCopy(source, target)
    ...
    local function copy(a, b)
        ...
        if mark[a] then return mark[a] end
        if not b then b = {} end
...
local function createLocal(key, effect, value, typeAnn)
    ...
    key.value = value
    key.typeAnn = typeAnn
    key.range = value and value.finish or typeAnn.finish
...
local Defs = {
    ...
    InterString = function(start, quote, strs, finish)
        ...
        if inter then obj.type = "interstring" end
    ...
    IfExp = function(ifStart, ifFinish, ifExp, thenStart, thenFinish, thenExp, elseIfs, elseStart, elseFinish, elseExp,
        finish)
        local obj = {
            type = "ifexp",
            start = ifStart,
            finish = finish,
            keyword = {ifStart, ifFinish - 1, thenStart, thenFinish - 1, elseStart, elseFinish - 1},
            filter = ifExp,
            [1] = thenExp
        }
        for i = 1, #elseIfs do
    ...
    ElseIfExp = function(ifStart, ifFinish, ifExp, thenStart, thenFinsih, thenExp, finish)
        return {
            ...
            start = ifStart,
            finish = finish,
            keyword = {ifStart, ifFinish - 1, thenStart, thenFinsih - 1},
            filter = ifExp,
            [1] = thenExp
        }
    end,
    ...
    Function = function(functionStart, functionFinish, name, generics, args, argsFinish, typeAnn, actions, endStart,
        endFinish)
        ...
        actions.keyword = {functionStart, functionFinish - 1, endStart, endFinish - 1}
    ...
    Do = function(start, actions, endA, endB)
        ...
        actions.keyword = {start, start + #"do" - 1, endA, endB - 1}
        checkMissEnd(start)
        return actions
    end,
    ...
    IfBlock = function(ifStart, ifFinish, exp, thenStart, thenFinish, actions, finish)
        ...
        actions.filter = exp
        actions.keyword = {ifStart, ifFinish - 1, thenStart, thenFinish - 1}
        return actions
    end,
    ElseIfBlock = function(elseifStart, elseifFinish, exp, thenStart, thenFinish, actions, finish)
        ...
        actions.finish = finish - 1
        actions.filter = exp
        actions.keyword = {elseifStart, elseifFinish - 1, thenStart, thenFinish - 1}
        return actions
    end,
    ElseBlock = function(elseStart, elseFinish, actions, finish)
        ...
        actions.finish = finish - 1
        actions.keyword = {elseStart, elseFinish - 1}
        return actions
    end,
    ...
    Loop = function(forA, forB, arg, typeAnn, steps, doA, doB, blockStart, block, endA, endB)
        ...
        block.max = steps[2]
        block.step = steps[3]
        block.keyword = {forA, forB - 1, doA, doB - 1, endA, endB - 1}
        checkMissEnd(forA)
        return block
    end,
    In = function(forA, forB, keys, inA, inB, exp, doA, doB, blockStart, block, endA, endB)
        ...
        block.finish = endB - 1
        block.keys = keys
        block.keyword = {forA, forB - 1, inA, inB - 1, doA, doB - 1, endA, endB - 1}
    ...
    While = function(whileA, whileB, filter, doA, doB, block, endA, endB)
        ...
        block.finish = endB - 1
        block.filter = filter
        block.keyword = {whileA, whileB - 1, doA, doB - 1, endA, endB - 1}
        checkMissEnd(whileA)
        return block
    end,
    Repeat = function(repeatA, repeatB, block, untilA, untilB, filter, finish)
        ...
        block.finish = finish
        block.filter = filter
        block.keyword = {repeatA, repeatB - 1, untilA, untilB - 1}
        return block
    end,
